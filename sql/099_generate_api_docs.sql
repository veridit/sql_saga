...
CREATE OR REPLACE TEMP VIEW funcs AS
    SELECT
        ...
        d.description,
        CASE
            WHEN p.proname IN ('add_era', 'drop_era', 'set_era_ephemeral_columns') THEN 1
        ...
        CASE
            WHEN p.proname IN ('add_era', 'drop_era', 'set_era_ephemeral_columns') THEN 'Era Management'
    ...
    WHERE
        ...
        AND p.proname NOT IN (
            'temporal_merge_plan', 'temporal_merge_plan_native', 'temporal_merge_execute',
            'temporal_merge_native_cache_stats', 'temporal_merge_native_cache_reset',
            'temporal_merge_executor_introspect',
