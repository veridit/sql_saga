...
SELECT  sql_saga.covers_without_gaps(tstzrange(valid_after,valid_to, '(]'), tstzrange('2017-11-27 06:00:00', '2017-11-27 12:00:00', '(]') ORDER BY valid_after)
...
SELECT  sql_saga.covers_without_gaps(tstzrange(valid_after,valid_to, '(]'), tstzrange('-infinity', '2017-11-27 20:00:00', '(]'))
FROM    timestamp_shifts
WHERE   job_id = 3;
...
SELECT  sql_saga.covers_without_gaps(tstzrange(valid_after,valid_to, '(]'), tstzrange('2017-11-27 03:00:00', 'infinity', '(]'))
FROM    timestamp_shifts
WHERE   job_id = 4;
