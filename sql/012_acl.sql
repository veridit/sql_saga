...
CREATE VIEW show_owners AS
    ...
    WHERE c.relnamespace = 'public'::regnamespace
      AND c.relname = ANY (ARRAY['owner_test', 'owner_test_history', 'owner_test_with_history', 'owner_test__for_portion_of_p', 'owner_test__current_p'])
    UNION ALL
...
CREATE TABLE owner_test (col text PRIMARY KEY, f date, u date);
...
SELECT sql_saga.drop_current_view('owner_test', 'p');
...
CREATE TABLE fpacl (col text PRIMARY KEY, f date, u date);
...
GRANT SELECT, UPDATE ON TABLE fpacl__current_p TO periods_acl_2; -- fail
