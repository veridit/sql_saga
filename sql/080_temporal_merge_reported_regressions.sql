...
-- A stable function to override CURRENT_DATE for deterministic testing
CREATE FUNCTION saga_repro.test_date() RETURNS date AS $$ SELECT '2026-01-01'::date $$ LANGUAGE sql;
...
CREATE TABLE saga_repro.target_with_audit (
    ...
    valid_from DATE NOT NULL DEFAULT saga_repro.test_date(),
...
-- ============================================================================
-- SCENARIO 10: ERA-LEVEL ephemeral_columns with add_era() auto-discovery
-- ============================================================================
-- This scenario tests that ephemeral_columns can be set at the ERA level via
-- add_era(), and that add_for_portion_of_view() will automatically discover
-- and use them WITHOUT needing to pass ephemeral_columns to the view.
--
-- This is the recommended approach for tables with audit columns that should
-- always be excluded from coalescing comparisons.
--
-- Expected: ephemeral_columns set via add_era() are auto-discovered by
-- add_for_portion_of_view() and temporal_merge, resulting in proper coalescing.
-- ============================================================================

\echo '\n==========================================================='
\echo 'SCENARIO 10: ERA-LEVEL ephemeral_columns with add_era() auto-discovery'
\echo '==========================================================='

SAVEPOINT scenario_era_level_ephemeral;

-- Create a table with audit columns (like legal_unit in statbus)
CREATE TABLE saga_repro.lu_era_ephemeral (
    id int GENERATED BY DEFAULT AS IDENTITY,
    name text NOT NULL,
    legal_form_id int NOT NULL DEFAULT 1,
    valid_range daterange NOT NULL,
    valid_from date NOT NULL,
    valid_until date,
    -- Audit columns: these will have DIFFERENT values across adjacent rows
    edit_at timestamptz NOT NULL DEFAULT statement_timestamp(),
    edit_by_user_id int NOT NULL DEFAULT 1
);

-- KEY: Set ephemeral_columns at the ERA level via add_era()
SELECT sql_saga.add_era('saga_repro.lu_era_ephemeral', 'valid_range',
    valid_from_column_name => 'valid_from',
    valid_until_column_name => 'valid_until',
...
SELECT era_name, ephemeral_columns FROM sql_saga.era 
WHERE table_name = 'lu_era_ephemeral';

ALTER TABLE saga_repro.lu_era_ephemeral ADD PRIMARY KEY (id, valid_range WITHOUT OVERLAPS);
SELECT sql_saga.add_unique_key('saga_repro.lu_era_ephemeral', ARRAY['id'], key_type => 'primary');
...
SELECT sql_saga.add_for_portion_of_view('saga_repro.lu_era_ephemeral', era_name => 'valid');

GRANT ALL ON saga_repro.lu_era_ephemeral TO sql_saga_unprivileged_user;
GRANT ALL ON saga_repro.lu_era_ephemeral__for_portion_of_valid TO sql_saga_unprivileged_user;

-- Insert two adjacent temporal rows for the SAME entity (id=1)
-- with different names AND different edit_at timestamps
INSERT INTO saga_repro.lu_era_ephemeral 
    (id, name, legal_form_id, valid_from, valid_until, edit_at, edit_by_user_id) 
VALUES
    (1, 'Name A', 1, '2023-01-01', '2024-01-01', '2023-01-15 10:00:00+00', 100),
    (1, 'Name B', 1, '2024-01-01', '2025-01-01', '2024-01-20 14:30:00+00', 200);

\echo '\n--- Initial State: Two adjacent rows, different names, different edit_at ---'
SELECT id, name, valid_from, valid_until, edit_at, edit_by_user_id
FROM saga_repro.lu_era_ephemeral
ORDER BY valid_from;

-- Now update the name to the SAME value across BOTH time periods
...
UPDATE saga_repro.lu_era_ephemeral__for_portion_of_valid
SET name = 'EQUATOR SOLUTIONS', valid_from = '2023-01-01', valid_until = '2025-01-01'
WHERE id = 1;

RESET ROLE;

\echo '\n--- After UPDATE via FOR_PORTION_OF (setting same name across full timeline) ---'
\echo '--- EXPECTED: 1 coalesced row with name=EQUATOR SOLUTIONS ---'
\echo '--- (auto-discovery of era-level ephemeral_columns enables coalescing) ---'
SELECT id, name, valid_from, valid_until, edit_at, edit_by_user_id
FROM saga_repro.lu_era_ephemeral
ORDER BY valid_from;

\echo '\n--- Verification: Count of result rows ---'
SELECT 
    count(*) as total_rows,
    count(DISTINCT name) as distinct_names
FROM saga_repro.lu_era_ephemeral;

-- The test passes if total_rows = 1 (coalesced via auto-discovery)

ROLLBACK TO SAVEPOINT scenario_era_level_ephemeral;


-- ============================================================================
-- SCENARIO 11: set_era_ephemeral_columns() allows post-creation configuration
-- ============================================================================
-- This tests that ephemeral_columns can be configured AFTER era creation
-- using the set_era_ephemeral_columns() function.
--
-- The workflow is:
-- 1. Create era WITHOUT ephemeral_columns
-- 2. Create FOR_PORTION_OF view (which will have no ephemeral_columns to discover)
-- 3. Demonstrate that coalescing FAILS (produces 2 rows)
-- 4. Call set_era_ephemeral_columns() to configure ephemeral columns
-- 5. Repeat the update and show that coalescing NOW WORKS (produces 1 row)
-- ============================================================================

\echo '\n==========================================================='
\echo 'SCENARIO 11: set_era_ephemeral_columns() post-creation configuration'
\echo '==========================================================='

SAVEPOINT scenario_set_ephemeral;

-- Create table with audit columns
CREATE TABLE saga_repro.lu_set_ephemeral (
    id int GENERATED BY DEFAULT AS IDENTITY,
    name text NOT NULL,
    valid_range daterange NOT NULL,
    valid_from date NOT NULL,
...
-- Add era WITHOUT ephemeral_columns
SELECT sql_saga.add_era('saga_repro.lu_set_ephemeral', 'valid_range',
    valid_from_column_name => 'valid_from',
    valid_until_column_name => 'valid_until');

-- Show era has NULL ephemeral_columns
\echo '\n--- Era initially has NULL ephemeral_columns ---'
SELECT era_name, ephemeral_columns FROM sql_saga.era 
WHERE table_name = 'lu_set_ephemeral';

ALTER TABLE saga_repro.lu_set_ephemeral ADD PRIMARY KEY (id, valid_range WITHOUT OVERLAPS);
SELECT sql_saga.add_unique_key('saga_repro.lu_set_ephemeral', ARRAY['id'], key_type => 'primary');

-- Create FOR_PORTION_OF view WITHOUT ephemeral_columns
-- At this point, no ephemeral_columns are configured anywhere
SELECT sql_saga.add_for_portion_of_view('saga_repro.lu_set_ephemeral', era_name => 'valid');

-- Insert test data with DIFFERENT audit values
INSERT INTO saga_repro.lu_set_ephemeral (id, name, valid_from, valid_until, edit_at, edit_by_user_id) VALUES
    (1, 'Name A', '2023-01-01', '2024-01-01', '2023-01-15 10:00:00+00', 100),
    (1, 'Name B', '2024-01-01', '2025-01-01', '2024-01-20 14:30:00+00', 200);

\echo '\n--- Initial state: 2 rows with different names and audit values ---'
SELECT id, name, valid_from, valid_until, edit_at, edit_by_user_id 
FROM saga_repro.lu_set_ephemeral ORDER BY valid_from;

-- Update both rows to same name via FOR_PORTION_OF
GRANT ALL ON saga_repro.lu_set_ephemeral TO sql_saga_unprivileged_user;
GRANT ALL ON saga_repro.lu_set_ephemeral__for_portion_of_valid TO sql_saga_unprivileged_user;

SET ROLE TO sql_saga_unprivileged_user;
UPDATE saga_repro.lu_set_ephemeral__for_portion_of_valid
SET name = 'UNIFIED NAME', valid_from = '2023-01-01', valid_until = '2025-01-01'
WHERE id = 1;
RESET ROLE;

\echo '\n--- After update WITHOUT ephemeral_columns: Should be 2 rows (no coalescing) ---'
SELECT id, name, valid_from, valid_until, edit_at, edit_by_user_id 
FROM saga_repro.lu_set_ephemeral ORDER BY valid_from;

SELECT COUNT(*) as row_count FROM saga_repro.lu_set_ephemeral;

-- Reset the data for the second test
DELETE FROM saga_repro.lu_set_ephemeral;
INSERT INTO saga_repro.lu_set_ephemeral (id, name, valid_from, valid_until, edit_at, edit_by_user_id) VALUES
    (1, 'Name A', '2023-01-01', '2024-01-01', '2023-01-15 10:00:00+00', 100),
    (1, 'Name B', '2024-01-01', '2025-01-01', '2024-01-20 14:30:00+00', 200);

-- NOW set ephemeral_columns via the setter function
SELECT sql_saga.set_era_ephemeral_columns(
    'saga_repro.lu_set_ephemeral'::regclass,
    era_name => 'valid',
    ephemeral_columns => ARRAY['edit_at', 'edit_by_user_id']
);

-- Show era now has ephemeral_columns set
\echo '\n--- Era now has ephemeral_columns set via set_era_ephemeral_columns() ---'
SELECT era_name, ephemeral_columns FROM sql_saga.era 
WHERE table_name = 'lu_set_ephemeral';

-- Update again - this time should coalesce
SET ROLE TO sql_saga_unprivileged_user;
UPDATE saga_repro.lu_set_ephemeral__for_portion_of_valid
SET name = 'UNIFIED NAME', valid_from = '2023-01-01', valid_until = '2025-01-01'
WHERE id = 1;
...
\echo '\n--- After update WITH ephemeral_columns set: Should be 1 row (coalesced) ---'
SELECT id, name, valid_from, valid_until, edit_at, edit_by_user_id 
FROM saga_repro.lu_set_ephemeral ORDER BY valid_from;

SELECT COUNT(*) as row_count FROM saga_repro.lu_set_ephemeral;

ROLLBACK TO SAVEPOINT scenario_set_ephemeral;


--------------------------------------------------------------------------------
-- SCENARIO 12: View-level ephemeral_columns overrides era-level defaults
--
-- This tests that ephemeral_columns specified on add_for_portion_of_view()
-- take precedence over era-level ephemeral_columns.
--
-- Resolution order:
-- 1. View-level ephemeral_columns (if not NULL) - takes precedence
-- 2. Era-level ephemeral_columns (fallback)
--
-- Test strategy:
-- 1. Create table with multiple audit columns: edit_at, edit_by_user_id, other_audit
-- 2. Add era WITH ephemeral_columns => ARRAY['edit_at'] (only edit_at is ephemeral)
-- 3. Create FOR_PORTION_OF view WITH ephemeral_columns => ARRAY['edit_at', 'edit_by_user_id']
--    (view overrides to include edit_by_user_id too)
-- 4. Insert test data where edit_at and edit_by_user_id have DIFFERENT values
-- 5. Update both rows to same name
-- 6. Expected: 1 coalesced row (view-level includes BOTH audit columns as ephemeral)
--------------------------------------------------------------------------------

\echo '\n==========================================================='
\echo 'SCENARIO 12: View-level ephemeral_columns overrides era-level defaults'
\echo '==========================================================='

SAVEPOINT scenario_view_override;

-- Create table with multiple audit columns
CREATE TABLE saga_repro.lu_override_test (
    id int GENERATED BY DEFAULT AS IDENTITY,
    name text NOT NULL,
    edit_at timestamptz NOT NULL,
    edit_by_user_id int NOT NULL,
...
-- Add era with PARTIAL ephemeral_columns (only edit_at)
SELECT sql_saga.add_era('saga_repro.lu_override_test', 'valid_range',
    valid_from_column_name => 'valid_from',
    valid_until_column_name => 'valid_until',
    ephemeral_columns => ARRAY['edit_at']);  -- Only edit_at is ephemeral at era level

\echo '\n--- Era-level ephemeral_columns: only edit_at ---'
SELECT era_name, ephemeral_columns FROM sql_saga.era WHERE table_name = 'lu_override_test';

ALTER TABLE saga_repro.lu_override_test ADD PRIMARY KEY (id, valid_range WITHOUT OVERLAPS);
SELECT sql_saga.add_unique_key('saga_repro.lu_override_test', ARRAY['id'], key_type => 'primary');

-- Create FOR_PORTION_OF view WITH override (includes edit_by_user_id too)
SELECT sql_saga.add_for_portion_of_view('saga_repro.lu_override_test', 
    era_name => 'valid',
    ephemeral_columns => ARRAY['edit_at', 'edit_by_user_id']);  -- View overrides to include both

\echo '\n--- View-level ephemeral_columns override: edit_at AND edit_by_user_id ---'
SELECT view_name, ephemeral_columns FROM sql_saga.updatable_view WHERE view_name = 'lu_override_test__for_portion_of_valid';

GRANT ALL ON saga_repro.lu_override_test TO sql_saga_unprivileged_user;
GRANT ALL ON saga_repro.lu_override_test__for_portion_of_valid TO sql_saga_unprivileged_user;

-- Insert test data with DIFFERENT values for BOTH audit columns
INSERT INTO saga_repro.lu_override_test (id, name, edit_at, edit_by_user_id, valid_from, valid_until) VALUES
    (1, 'Name A', '2023-01-15 10:00:00+00', 100, '2023-01-01', '2024-01-01'),
    (1, 'Name B', '2024-01-20 14:30:00+00', 200, '2024-01-01', '2025-01-01');

\echo '\n--- Initial state: 2 rows with different names AND different audit values ---'
SELECT id, name, valid_from, valid_until, edit_at, edit_by_user_id 
FROM saga_repro.lu_override_test ORDER BY valid_from;

-- Update via FOR_PORTION_OF view (which has the override)
SET ROLE TO sql_saga_unprivileged_user;
UPDATE saga_repro.lu_override_test__for_portion_of_valid
SET name = 'UNIFIED NAME', valid_from = '2023-01-01', valid_until = '2025-01-01'
WHERE id = 1;
RESET ROLE;

\echo '\n--- After update with VIEW-LEVEL override (both audit cols ephemeral): Should be 1 row ---'
SELECT id, name, valid_from, valid_until, edit_at, edit_by_user_id 
FROM saga_repro.lu_override_test ORDER BY valid_from;

SELECT COUNT(*) as row_count FROM saga_repro.lu_override_test;

-- The test passes if we get 1 row (coalesced) because the VIEW-LEVEL ephemeral_columns
-- includes both edit_at AND edit_by_user_id, even though ERA-LEVEL only had edit_at.

ROLLBACK TO SAVEPOINT scenario_view_override;
