...
-- ============================================================================
-- Regression: REPLACE_FOR_PORTION_OF with extending source range
-- ============================================================================
-- When source range extends beyond target, the extending segment should be
-- ignored for FOR_PORTION_OF modes. REPLACE overwrites all columns (unlike
-- PATCH which only overwrites supplied columns), but the extending portion
-- should still not create a new INSERT.
-- ============================================================================
BEGIN;
\echo '----------------------------------------------------------------------------'
\echo 'Regression: REPLACE_FOR_PORTION_OF extending source range'
\echo '----------------------------------------------------------------------------'

CREATE SCHEMA replace_extend;
CREATE TABLE replace_extend.legal_unit (
    id int GENERATED BY DEFAULT AS IDENTITY,
    name text NOT NULL,
    address text,
    enterprise_id integer,
...
SELECT sql_saga.add_era('replace_extend.legal_unit', 'valid_range',
    valid_from_column_name => 'valid_from',
    valid_until_column_name => 'valid_until');
ALTER TABLE replace_extend.legal_unit ADD PRIMARY KEY (id, valid_range WITHOUT OVERLAPS);
SELECT sql_saga.add_unique_key('replace_extend.legal_unit', ARRAY['id'], key_type => 'primary');
SELECT sql_saga.add_unique_key('replace_extend.legal_unit', ARRAY['name'], key_type => 'natural');

\echo '--- Step 1: Seed with full data ---'
CREATE TEMP TABLE source_replace_seed (
    row_id int GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    name text,
    address text,
    enterprise_id integer,
...
INSERT INTO source_replace_seed (name, address, enterprise_id, valid_from, valid_until) VALUES
    ('Company One', 'Street 1', NULL, '2023-01-01', '2023-12-31'),
    ('Company Two', 'Street 2', NULL, '2023-01-01', '2023-12-31');

CALL sql_saga.temporal_merge(
    target_table => 'replace_extend.legal_unit',
    source_table => 'source_replace_seed',
    mode => 'MERGE_ENTITY_UPSERT'
);

\echo '--- After seed ---'
SELECT id, name, address, enterprise_id, valid_from, valid_until
FROM replace_extend.legal_unit ORDER BY id, valid_from;

\echo '--- Step 2: REPLACE with fewer columns and extending range ---'
-- Source has name + enterprise_id (no address column).
-- Source range [2023-01-01, infinity) extends beyond target [2023-01-01, 2023-12-31).
-- Expected: overlapping portion is replaced (address becomes NULL), no extending INSERT.
CREATE TEMP TABLE source_replace (
    row_id int GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    name text,
    enterprise_id integer,
    valid_from date,
...
INSERT INTO source_replace (name, enterprise_id, valid_from, valid_until) VALUES
    ('Company Two', 42, '2023-01-01', 'infinity');

CALL sql_saga.temporal_merge(
    target_table => 'replace_extend.legal_unit',
    source_table => 'source_replace',
...
\echo '--- Verify: Company One unchanged ---'
SELECT count(*) AS company_one_rows
FROM replace_extend.legal_unit WHERE name = 'Company One';

\echo '--- Verify: Company Two replaced, no extending row ---'
SELECT count(*) AS company_two_rows,
       count(*) FILTER (WHERE enterprise_id = 42) AS replaced_rows,
       count(*) FILTER (WHERE valid_from >= '2023-12-31') AS extending_rows
FROM replace_extend.legal_unit WHERE name = 'Company Two';

ROLLBACK;

-- ============================================================================
-- Regression: DELETE_FOR_PORTION_OF with extending source range
-- ============================================================================
-- When source range extends beyond target, the extending segment should not
-- cause any additional deletions outside the overlapping portion.
-- ============================================================================
BEGIN;
\echo '----------------------------------------------------------------------------'
\echo 'Regression: DELETE_FOR_PORTION_OF extending source range'
\echo '----------------------------------------------------------------------------'

CREATE SCHEMA delete_extend;
CREATE TABLE delete_extend.legal_unit (
    id int GENERATED BY DEFAULT AS IDENTITY,
    name text NOT NULL,
    address text,
    enterprise_id integer,
...
SELECT sql_saga.add_era('delete_extend.legal_unit', 'valid_range',
    valid_from_column_name => 'valid_from',
    valid_until_column_name => 'valid_until');
ALTER TABLE delete_extend.legal_unit ADD PRIMARY KEY (id, valid_range WITHOUT OVERLAPS);
SELECT sql_saga.add_unique_key('delete_extend.legal_unit', ARRAY['id'], key_type => 'primary');
SELECT sql_saga.add_unique_key('delete_extend.legal_unit', ARRAY['name'], key_type => 'natural');

\echo '--- Step 1: Seed with full data ---'
CREATE TEMP TABLE source_delete_seed (
    row_id int GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    name text,
    address text,
    enterprise_id integer,
...
INSERT INTO source_delete_seed (name, address, enterprise_id, valid_from, valid_until) VALUES
    ('Company One', 'Street 1', NULL, '2023-01-01', '2023-12-31'),
    ('Company Two', 'Street 2', NULL, '2023-01-01', '2023-12-31');

CALL sql_saga.temporal_merge(
    target_table => 'delete_extend.legal_unit',
    source_table => 'source_delete_seed',
    mode => 'MERGE_ENTITY_UPSERT'
);

\echo '--- After seed ---'
SELECT id, name, address, enterprise_id, valid_from, valid_until
FROM delete_extend.legal_unit ORDER BY id, valid_from;

\echo '--- Step 2: DELETE with extending range targeting Company Two ---'
-- Source range [2023-01-01, infinity) extends beyond target [2023-01-01, 2023-12-31).
-- Expected: Company Two deleted in overlapping range, no extending effect.
CREATE TEMP TABLE source_delete (
    row_id int GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    name text,
    valid_from date,
    valid_until date
) ON COMMIT DROP;
INSERT INTO source_delete (name, valid_from, valid_until) VALUES
    ('Company Two', '2023-01-01', 'infinity');

CALL sql_saga.temporal_merge(
    target_table => 'delete_extend.legal_unit',
    source_table => 'source_delete',
...
\echo '--- Verify: Company One unchanged ---'
SELECT count(*) AS company_one_rows
FROM delete_extend.legal_unit WHERE name = 'Company One';

\echo '--- Verify: Company Two deleted, no extending rows ---'
SELECT count(*) AS company_two_rows,
       count(*) FILTER (WHERE valid_from >= '2023-12-31') AS extending_rows
FROM delete_extend.legal_unit WHERE name = 'Company Two';
