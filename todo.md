...
- [ ] **Fix `temporal_merge` FK violation during multi-batch ETL:** The test `56_temporal_merge-minimal-copying-architecture` fails with a foreign key violation in batch 3. Investigate why the `INSERT`-then-`UPDATE` strategy fails in this specific cross-batch scenario, even though the underlying architectural principles have been validated.
- [x] **Validate `temporal_merge` architectural assumptions with a low-level test:** Created a new, isolated test (`57_temporal_merge_validate_timely_strategy.sql`) that uses only raw SQL to prove the core design principles: that an `INSERT`-then-`UPDATE` sequence is a valid strategy for SCD Type 2 changes when used with deferred `EXCLUDE` constraints, and that `AFTER` triggers correctly see the consistent MVCC snapshot at each step.
