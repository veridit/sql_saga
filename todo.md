...
- [x] **Finalize advanced ETL reference implementation:** The `56_...` test is now a complete reference for a robust, minimal-copying ETL architecture. It demonstrates ID back-propagation, dynamic metadata-driven procedures that correctly filter source data, handling of natural keys, non-temporal targets, and ephemeral metadata columns.
